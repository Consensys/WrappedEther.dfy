...
module transfer {
	...
	requires st'.Operands() == 1
	{
		...
		st := Dup(st,1);
		//|fp=0x0060|0x04*,_,0x3b0*,_|
		st := Dup(st,1);
		//|fp=0x0060|_,0x04*,_,0x3b0*,_|
		st := CallDataLoad(st);
		//|fp=0x0060|_,0x04*,_,0x3b0*,_|
		st := PushN(st,20,0xffffffffffffffffffffffffffffffffffffffff);
		//|fp=0x0060|_,_,0x04*,_,0x3b0*,_|
		st := And(st);
	...
	requires (st'.Peek(1) == 0x4 && st'.Peek(3) == 0x3b0)
	{
		...
		st := Push1(st,0x20);
		//|fp=0x0060|0x20*,0x04*,_,_,0x3b0*,_|
		assert (st.Peek(0) + st.Peek(1)) <= (MAX_U256 as u256);
		//|fp=0x0060|0x20*,0x04*,_,_,0x3b0*,_|
		st := Add(st);
		//|fp=0x0060|0x24*,_,_,0x3b0*,_|
		st := Swap(st,1);
		//|fp=0x0060|_,0x24*,_,0x3b0*,_|
		st := Swap(st,2);
		//|fp=0x0060|_,0x24*,_,0x3b0*,_|
		st := Swap(st,1);
		//|fp=0x0060|0x24*,_,_,0x3b0*,_|
		st := Dup(st,1);
		//|fp=0x0060|0x24*,0x24*,_,_,0x3b0*,_|
		st := CallDataLoad(st);
	...
	{
		var st := st';
		//|fp=0x0060|_*,0x24*,_,_,0x3b0*,_|
		st := Swap(st,1);
		//|fp=0x0060|0x24*,_*,_,_,0x3b0*,_|
		st := Push1(st,0x20);
		//|fp=0x0060|0x20*,0x24*,_*,_,_,0x3b0*,_|
		assert (st.Peek(0) + st.Peek(1)) <= (MAX_U256 as u256);
		//|fp=0x0060|_,_,_*,_,_,0x3b0*,_|
		st := Add(st);
	...
	requires (st'.Peek(2) == 0x3b0)
	{
		...
		//|fp=0x0060|_,_*,_,0x3b0*,_|
		st := Push2(st,0x0bdb);
		//|fp=0x0060|0xbdb*,_,_*,_,0x3b0*,_|
		st := Caller(st);
		//|fp=0x0060|_,0xbdb*,_,_*,_,0x3b0*,_|
		st := Dup(st,5);
		//|fp=0x0060|_,_,0xbdb*,_,_*,_,0x3b0*,_|
		st := Dup(st,5);
		//|fp=0x0060|_*,_,_,0xbdb*,_,_,_,0x3b0*,_|
		st := Push2(st,0x068c);
		//|fp=0x0060|0x68c*,_*,_,_,0xbdb*,_,_,_,0x3b0*,_|
