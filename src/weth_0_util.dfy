...
module util {
	...
	// from 3ca: //|fp=0x0060|0x3d2,callSig==0xd0e30db0| i.e from deposit
	// from a3 -> af: |fp=0x0060|0x00b7,callSig|
	// from 00 -> af: |fp=0x0060|0x00b7|
	...
	requires st'.Operands() == 2 ==> ((st'.Peek(0) == 0xb7) || (st'.Peek(0) == 0x3d2))
	{
		...
		//|fp=0x0060|0xb7|
		st := CallValue(st);
		//|fp=0x0060|callval,0xb7,_|
		//|fp=0x0060|_,0x3d2,_|
		//|fp=0x0060|_,0xb7|
		st := Push1(st,0x03);
		//|fp=0x0060|0x03,callval,0xb7,_|
		//|fp=0x0060|0x03,_,0x3d2,_|
		//|fp=0x0060|0x03,_,0xb7|
		st := Push1(st,0x00);
		//|fp=0x0060|0x00,0x03,callval,0xb7,_|
		...
		//|fp=0x0060|0xffffffffffffffffffffffffffffffffffffffff,caller,0x00,0x03,callVal,0xb7,_|
	...
	requires st'.Operands() == 7 ==> ((st'.Peek(5) == 0xb7) || (st'.Peek(5) == 0x3d2))
	{
		...
		//|fp=0x0060|_,0x00,0x03,_,0xb7|
		st := Dup(st,2);
		//|fp=0x0060|0x00,caller,0x00,0x03,callVal,0xb7,_|
		//|fp=0x0060|0x00,_,0x00,0x03,_,0x3d2,_|
		//|fp=0x0060|0x00,_,0x00,0x03,_,0xb7|
		st := MStore(st);
		assert {:split_here} true;
		//|fp=0x0060|0x00,0x03,callVal,0xb7,_| i.e. st.Read(0x00) == caller
		//|fp=0x0060|0x00,0x03,_,0x3d2,_|
		//|fp=0x0060|0x00,0x03,_,0xb7|
		st := Push1(st,0x20);
		//|fp=0x0060|0x20,0x00,0x03,callVal,0xb7,_|
		//|fp=0x0060|0x20,0x00,0x03,_,0x3d2,_|
		//|fp=0x0060|0x20,0x00,0x03,_,0xb7|
		assert (st.Peek(0) == 0x20 && st.Peek(1) == 0x0); 
		assert (st.Peek(0) + st.Peek(1)) <= (MAX_U256 as u256);
		st := Add(st);
		assert {:split_here} true;
		assert (st.Peek(0) == 0x20);
		assert st.Operands() == 4 ==> ((st.Peek(3) == 0xb7));
		assert st.Operands() == 5 ==> ((st.Peek(3) == 0xb7) || (st.Peek(3) == 0x3d2));
		//|fp=0x0060|0x20,0x03,callVal,0xb7,_|
		//|fp=0x0060|0x20,0x03,_,0x3d2,_|
		//|fp=0x0060|0x20,0x03,_,0xb7|
		st := Swap(st,1);
		//|fp=0x0060|0x03,0x20,callVal,0xb7,_|
		//|fp=0x0060|0x03,0x20,_,0x3d2,_|
		//|fp=0x0060|0x03,0x20,_,0xb7|
		st := Dup(st,2);
		//|fp=0x0060|0x20,0x03,0x20,callVal,0xb7,_|
		//|fp=0x0060|0x20,0x03,0x20,_,0x3d2,_|
		//|fp=0x0060|0x20,0x03,0x20,_,0xb7|
		st := MStore(st);
		assert {:split_here} true; // i.e i.e. st.Read(0x20) == 0x03
		assert (st.Peek(0) == 0x20);
		assert st.Operands() == 3 ==> ((st.Peek(2) == 0xb7));
		assert st.Operands() == 4 ==> ((st.Peek(2) == 0xb7) || (st.Peek(2) == 0x3d2));
	...
	requires st'.Operands() == 7 ==> ((st'.Peek(5) == 0xb7) || (st'.Peek(5) == 0x3d2))
	{
		...
		//|fp=0x0060|_,_,0x00,_,_,0x3d2,_|
		//|fp=0x0060|_,_,0x00,_,_,0xb7|
		assume st.Peek(1) == st.evm.context.callValue == 0;
		assert (st.Peek(0) + st.Peek(1)) <= (MAX_U256 as u256); // not possible to assert unless call value is 0 ??
		st := Add(st);
	...
	requires st'.Operands() == 7 ==> ((st'.Peek(5) == 0xb7) || (st'.Peek(5) == 0x3d2))
	{
		...
		st := MStore(st);
		assert {:split_here} true;
		assert st.Operands() == 6 ==> ((st.Peek(5) == 0xb7));
		assert st.Operands() == 7 ==> ((st.Peek(5) == 0xb7) || (st.Peek(5) == 0x3d2));
		//|fp=0x0060|0x60,0x60,_,0xe1fffcc4923d04b559f4d29a8bfc6cda4eb5b0d3c460751c2402c5c5cc9109c,_,0xb7,_|
		//|fp=0x0060|0x60,0x60,_,0xe1fffcc4923d04b559f4d29a8bfc6cda4eb5b0d3c460751c2402c5c5cc9109c,_,0x3d2,_|
		//|fp=0x0060|0x60,0x60,_,0xe1fffcc4923d04b559f4d29a8bfc6cda4eb5b0d3c460751c2402c5c5cc9109c,_,0xb7|
		st := Push1(st,0x20);
	...
	// from 223: |fp=0x0060|data[0x44],addr2,addr,0x229,callSig| & call value == 0 
