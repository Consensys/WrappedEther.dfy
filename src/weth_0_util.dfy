...
module util {
	...
	requires st'.Operands() == 7 ==> ((st'.Peek(5) == 0xb7) || (st'.Peek(5) == 0x3d2))
	{
		...
		//|fp=0x0060|0x00*,_,_*,0xb7*|
		st := Push1(st,0x20);
		//|fp=0x0060|0x20*,0x00*,_,_*,0xb7*,_|
		//|fp=0x0060|0x20*,0x00*,_,_*,0x3d2*,_|
		//|fp=0x0060|0x20*,0x00*,_,_*,0xb7*|
		assert (st.Peek(0) + st.Peek(1)) <= (MAX_U256 as u256);
		//|fp=0x0060|0x20*,0x00*,_,_*,0xb7*,_|
		//|fp=0x0060|0x20*,0x00*,_,_*,0x3d2*,_|
		//|fp=0x0060|0x20*,0x00*,_,_*,0xb7*|
		st := Add(st);
		//|fp=0x0060|0x20*,_,_*,0xb7*,_|
		//|fp=0x0060|0x20*,_,_*,0x3d2*,_|
		//|fp=0x0060|0x20*,_,_*,0xb7*|
		st := Swap(st,1);
		//|fp=0x0060|_,0x20*,_*,0xb7*,_|
		//|fp=0x0060|_,0x20*,_*,0x3d2*,_|
		//|fp=0x0060|_,0x20*,_*,0xb7*|
		st := Dup(st,2);
		//|fp=0x0060|_,_,0x20*,_*,0xb7*,_|
		//|fp=0x0060|_,_,0x20*,_*,0x3d2*,_|
		//|fp=0x0060|_,_,0x20*,_*,0xb7*|
		st := MStore(st);
	...
	requires st'.Operands() == 4 ==> ((st'.Peek(2) == 0xb7) || (st'.Peek(2) == 0x3d2))
	{
		...
		st := Dup(st,3);
		//|fp=0x0060|_*,_,_*,_,0xb7*,_|
		//|fp=0x0060|_*,_,_*,_,0x3d2*,_|
		//|fp=0x0060|_*,_,_*,_,0xb7*|
		st := Dup(st,3);
		//|fp=0x0060|_*,_*,_,_,_,0xb7*,_|
		//|fp=0x0060|_*,_*,_,_,_,0x3d2*,_|
		//|fp=0x0060|_*,_*,_,_,_,0xb7*|
		st := SLoad(st);
	...
	requires st'.Operands() == 2 ==> ((st'.Peek(0) == 0xb7) || (st'.Peek(0) == 0x3d2))
	{
		...
		//|fp=0x0060|_,_,_,0xb7*|
		st := Push1(st,0x40);
		//|fp=0x0060|0x40*,_,_,_,0xb7*,_|
		//|fp=0x0060|0x40*,_,_,_,0x3d2*,_|
		//|fp=0x0060|0x40*,_,_,_,0xb7*|
		st := MLoad(st);
		//|fp=0x0060|0x60*,_,_,_,0xb7*,_|
		//|fp=0x0060|0x60*,_,_,_,0x3d2*,_|
		//|fp=0x0060|0x60*,_,_,_,0xb7*|
		st := Dup(st,1);
	...
	{
		var st := st';
		//|fp=0x0060|0x60*,_,_,_,_,0xb7*,_|
		//|fp=0x0060|0x60*,_,_,_,_,0x3d2*,_|
		//|fp=0x0060|0x60*,_,_,_,_,0xb7*|
		st := Dup(st,3);
		//|fp=0x0060|_,0x60*,_,_,_,_,0xb7*,_|
		//|fp=0x0060|_,0x60*,_,_,_,_,0x3d2*,_|
		//|fp=0x0060|_,0x60*,_,_,_,_,0xb7*|
		st := Dup(st,2);
		//|fp=0x0060|_,_,0x60*,_,_,_,_,0xb7*,_|
		//|fp=0x0060|_,_,0x60*,_,_,_,_,0x3d2*,_|
		//|fp=0x0060|_,_,0x60*,_,_,_,_,0xb7*|
		st := MStore(st);
		//|fp=0x0060|0x60*,_,_,_,_,0xb7*,_|
		//|fp=0x0060|0x60*,_,_,_,_,0x3d2*,_|
		//|fp=0x0060|0x60*,_,_,_,_,0xb7*|
		st := Push1(st,0x20);
		//|fp=0x0060|0x20*,0x60*,_,_,_,_,0xb7*,_|
		//|fp=0x0060|0x20*,0x60*,_,_,_,_,0x3d2*,_|
		//|fp=0x0060|0x20*,0x60*,_,_,_,_,0xb7*|
		assert (st.Peek(0) + st.Peek(1)) <= (MAX_U256 as u256);
		//|fp=0x0060|0x20*,0x60*,_,_,_,_,0xb7*,_|
		//|fp=0x0060|0x20*,0x60*,_,_,_,_,0x3d2*,_|
		//|fp=0x0060|0x20*,0x60*,_,_,_,_,0xb7*|
		st := Add(st);
		//|fp=0x0060|0x80*,_,_,_,_,0xb7*,_|
		//|fp=0x0060|0x80*,_,_,_,_,0x3d2*,_|
		//|fp=0x0060|0x80*,_,_,_,_,0xb7*|
		st := Swap(st,2);
		//|fp=0x0060|_,_,0x80*,_,_,0xb7*,_|
		//|fp=0x0060|_,_,0x80*,_,_,0x3d2*,_|
		//|fp=0x0060|_,_,0x80*,_,_,0xb7*|
		st := Pop(st);
		//|fp=0x0060|_,0x80*,_,_,0xb7*,_|
		//|fp=0x0060|_,0x80*,_,_,0x3d2*,_|
		//|fp=0x0060|_,0x80*,_,_,0xb7*|
		st := Pop(st);
	...
	{
		var st := st';
		//|fp=0x0060|0x80*,_,_,0xb7*,_|
		//|fp=0x0060|0x80*,_,_,0x3d2*,_|
		//|fp=0x0060|0x80*,_,_,0xb7*|
		st := Push1(st,0x40);
		//|fp=0x0060|0x40*,0x80*,_,_,0xb7*,_|
		//|fp=0x0060|0x40*,0x80*,_,_,0x3d2*,_|
		//|fp=0x0060|0x40*,0x80*,_,_,0xb7*|
		st := MLoad(st);
		//|fp=0x0060|0x60*,0x80*,_,_,0xb7*,_|
		//|fp=0x0060|0x60*,0x80*,_,_,0x3d2*,_|
		//|fp=0x0060|0x60*,0x80*,_,_,0xb7*|
		st := Dup(st,1);
		//|fp=0x0060|_,0x60*,0x80*,_,_,0xb7*,_|
		//|fp=0x0060|_,0x60*,0x80*,_,_,0x3d2*,_|
		//|fp=0x0060|_,0x60*,0x80*,_,_,0xb7*|
		st := Swap(st,2);
		//|fp=0x0060|0x80*,0x60*,_,_,_,0xb7*,_|
		//|fp=0x0060|0x80*,0x60*,_,_,_,0x3d2*,_|
		//|fp=0x0060|0x80*,0x60*,_,_,_,0xb7*|
		assert st.Peek(1) <= st.Peek(0);
		//|fp=0x0060|_,_,_,_,_,0xb7*,_|
		//|fp=0x0060|_,_,_,_,_,0x3d2*,_|
		//|fp=0x0060|_,_,_,_,_,0xb7*|
	...
	{
		var st := st';
		//|fp=0x0060|_*,_,_,0xbdb*,_,_,_,0x3b0*,_|
		//|fp=0x0060|_*,_,_,0x229*,_|
		st := JumpDest(st);
		//|fp=0x0060|_*,_,_,0xbdb*,_,_,_,0x3b0*,_|
		//|fp=0x0060|_*,_,_,0x229*,_|
		st := Push1(st,0x00);
		//|fp=0x0060|_,_*,_,_,0xbdb*,_,_,_,0x3b0*,_|
		//|fp=0x0060|_,_*,_,_,0x229*,_|
		st := Dup(st,2);
		//|fp=0x0060|_,_,_*,_,_,0xbdb*,_,_,_,0x3b0*,_|
		//|fp=0x0060|_,_,_*,_,_,0x229*,_|
		st := Push1(st,0x03);
		//|fp=0x0060|_,_,_,_*,_,_,0xbdb*,_,_,_,0x3b0*,_|
		//|fp=0x0060|_,_,_,_*,_,_,0x229*,_|
		st := Push1(st,0x00);
		//|fp=0x0060|0x00*,_,_,_,_*,_,_,0xbdb*,_,_,_,0x3b0*,_|
		//|fp=0x0060|0x00*,_,_,_,_*,_,_,0x229*,_|
		st := Dup(st,7);
		//|fp=0x0060|_,0x00*,_,_,_,_*,_,_,0xbdb*,_,_,_,0x3b0*,_|
		//|fp=0x0060|_,0x00*,_,_,_,_*,_,_,0x229*,_|
		st := PushN(st,20,0xffffffffffffffffffffffffffffffffffffffff);
		//|fp=0x0060|_,_,0x00*,_,_,_,_*,_,_,0xbdb*,_,_,_,0x3b0*,_|
		//|fp=0x0060|_,_,0x00*,_,_,_,_*,_,_,0x229*,_|
		st := And(st);
	...
	requires st'.Operands() == 14 ==> ((st'.Peek(8) == 0xbdb && st'.Peek(12) == 0x3b0))
	{
		...
		//|fp=0x0060|0x00*,_,_,_,_*,_,_,0x229*,_|
		st := Push1(st,0x20);
		//|fp=0x0060|0x20*,0x00*,_,_,_,_*,_,_,0xbdb*,_,_,_,0x3b0*,_|
		//|fp=0x0060|0x20*,0x00*,_,_,_,_*,_,_,0x229*,_|
		assert (st.Peek(0) + st.Peek(1)) <= (MAX_U256 as u256);
		//|fp=0x0060|0x20*,0x00*,_,_,_,_*,_,_,0xbdb*,_,_,_,0x3b0*,_|
		//|fp=0x0060|0x20*,0x00*,_,_,_,_*,_,_,0x229*,_|
		st := Add(st);
		//|fp=0x0060|0x20*,_,_,_,_*,_,_,0xbdb*,_,_,_,0x3b0*,_|
		//|fp=0x0060|0x20*,_,_,_,_*,_,_,0x229*,_|
		st := Swap(st,1);
		//|fp=0x0060|_,0x20*,_,_,_*,_,_,0xbdb*,_,_,_,0x3b0*,_|
		//|fp=0x0060|_,0x20*,_,_,_*,_,_,0x229*,_|
		st := Dup(st,2);
	...
	{
		var st := st';
		//|fp=0x0060|_,_,0x20*,_,_,_*,_,_,0xbdb*,_,_,_,0x3b0*,_|
		//|fp=0x0060|_,_,0x20*,_,_,_*,_,_,0x229*,_|
		st := MStore(st);
		//|fp=0x0060|0x20*,_,_,_*,_,_,0xbdb*,_,_,_,0x3b0*,_|
		//|fp=0x0060|0x20*,_,_,_*,_,_,0x229*,_|
		st := Push1(st,0x20);
		//|fp=0x0060|0x20*,0x20*,_,_,_*,_,_,0xbdb*,_,_,_,0x3b0*,_|
		//|fp=0x0060|0x20*,0x20*,_,_,_*,_,_,0x229*,_|
		assert (st.Peek(0) + st.Peek(1)) <= (MAX_U256 as u256);
		//|fp=0x0060|_,_,_,_,_*,_,_,0xbdb*,_,_,_,0x3b0*,_|
		//|fp=0x0060|_,_,_,_,_*,_,_,0x229*,_|
		st := Add(st);
		//|fp=0x0060|_,_,_,_*,_,_,0xbdb*,_,_,_,0x3b0*,_|
		//|fp=0x0060|_,_,_,_*,_,_,0x229*,_|
		st := Push1(st,0x00);
	...
	requires st'.Operands() == 11 ==> ((st'.Peek(5) == 0xbdb && st'.Peek(9) == 0x3b0))
	{
		...
		//|fp=0x0060|_,_,_*,_,_,0x229*,_|
		st := Push2(st,0x06dc);
		//|fp=0x0060|0x6dc*,_,_,_*,_,_,0xbdb*,_,_,_,0x3b0*,_|
		//|fp=0x0060|0x6dc*,_,_,_*,_,_,0x229*,_|
		assume st.IsJumpDest(0x6dc);
		st := JumpI(st);
		if st.PC() == 0x6dc { st := block_0_0x06dc(st); return st;}
		//|fp=0x0060|_,_,_,_,_,_,_,_,_,_|
		//|fp=0x0060|_,_,_,_,_,_|
		st := Push1(st,0x00);
		//|fp=0x0060|_,_,_,_,_,_,_,_,_,_,_|
		//|fp=0x0060|_,_,_,_,_,_,_|
		st := Dup(st,1);
		//|fp=0x0060|_,_,_,_,_,_,_,_,_,_,_,_|
		//|fp=0x0060|_,_,_,_,_,_,_,_|
		st := Revert(st);
		return st;
	}
	...
	requires st'.Operands() == 11 ==> ((st'.Peek(5) == 0xbdb && st'.Peek(9) == 0x3b0))
	{
		...
		//|fp=0x0060|_,_,_,_*,_,_,0x229*,_|
		st := Push2(st,0x07b4);
		//|fp=0x0060|0x7b4*,_,_,_,_*,_,_,0xbdb*,_,_,_,0x3b0*,_|
		//|fp=0x0060|0x7b4*,_,_,_,_*,_,_,0x229*,_|
	...
	requires st'.Operands() == 11 ==> ((st'.Peek(5) == 0xbdb && st'.Peek(9) == 0x3b0))
	{
		...
		//|fp=0x0060|_,_,_*,_,_,0x229*,_|
		st := Push2(st,0x08cf);
		//|fp=0x0060|0x8cf*,_,_,_*,_,_,0xbdb*,_,_,_,0x3b0*,_|
		//|fp=0x0060|0x8cf*,_,_,_*,_,_,0x229*,_|
	...
	requires st'.Operands() in {10,14}
	// Static stack items
	requires (st'.Peek(0) == 0x20 && st'.Peek(1) == 0x0)
	// Dynamic stack items
	requires st'.Operands() == 10 ==> ((st'.Peek(8) == 0x229))
	requires st'.Operands() == 14 ==> ((st'.Peek(8) == 0xbdb && st'.Peek(12) == 0x3b0))
	{
	...
	requires st'.Operands() == 12 ==> ((st'.Peek(6) == 0xbdb && st'.Peek(10) == 0x3b0))
	{
		...
		//|fp=0x0060|_,_,_*,_,_,0x229*,_|
		st := Push2(st,0x0844);
		//|fp=0x0060|0x844*,_,_,_*,_,_,0xbdb*,_,_,_,0x3b0*,_|
		//|fp=0x0060|0x844*,_,_,_*,_,_,0x229*,_|
	...
	requires st'.Operands() == 14 ==> ((st'.Peek(8) == 0xbdb && st'.Peek(12) == 0x3b0))
	{
		...
		//|fp=0x0060|0x20*,0x20*,_*,_,_*,_,_,0x229*,_|
		st := Add(st);
		//|fp=0x0060|0x40*,_*,_,_*,_,_,0xbdb*,_,_,_,0x3b0*,_|
		//|fp=0x0060|0x40*,_*,_,_*,_,_,0x229*,_|
		st := Push1(st,0x00);
		//|fp=0x0060|0x00*,0x40*,_*,_,_*,_,_,0xbdb*,_,_,_,0x3b0*,_|
		//|fp=0x0060|0x00*,0x40*,_*,_,_*,_,_,0x229*,_|
		st := Keccak256(st);
		//|fp=0x0060|_*,_*,_,_*,_,_,0xbdb*,_,_,_,0x3b0*,_|
		//|fp=0x0060|_*,_*,_,_*,_,_,0x229*,_|
		st := Push1(st,0x00);
		...
		//|fp=0x0060|_,_*,_*,_,_*,_,_,0x229*,_|
		st := Dup(st,3);
		//|fp=0x0060|_*,_,_*,_,_,_*,_,_,0xbdb*,_,_,_,0x3b0*,_|
		//|fp=0x0060|_*,_,_*,_,_,_*,_,_,0x229*,_|
		st := Dup(st,3);
	...
	{
		var st := st';
		//|fp=0x0060|_*,_*,_,_,_,_,_*,_,_,0xbdb*,_,_,_,0x3b0*,_|
		//|fp=0x0060|_*,_*,_,_,_,_,_*,_,_,0x229*,_|
		st := SLoad(st);
		//|fp=0x0060|_*,_*,_,_,_,_,_*,_,_,0xbdb*,_,_,_,0x3b0*,_|
		//|fp=0x0060|_*,_*,_,_,_,_,_*,_,_,0x229*,_|
		assert st.Peek(1) <= st.Peek(0);
	...
	requires st'.Operands() in {9,13}
	// Static stack items
	requires (st'.Peek(0) == 0x20 && st'.Peek(1) == 0x20)
	// Dynamic stack items
	requires st'.Operands() == 9 ==> ((st'.Peek(7) == 0x229))
	requires st'.Operands() == 13 ==> ((st'.Peek(7) == 0xbdb && st'.Peek(11) == 0x3b0))
	{
	...
	requires st'.Operands() in {11,15}
	// Static stack items
	requires (st'.Peek(0) == 0x40)
	// Dynamic stack items
	requires st'.Operands() == 11 ==> ((st'.Peek(9) == 0x229))
	requires st'.Operands() == 15 ==> ((st'.Peek(9) == 0xbdb && st'.Peek(13) == 0x3b0))
	{
		var st := st';
		//|fp=0x0060|0x40*,_,_,_,_,_,_,_,_,0xbdb*,_,_,_,0x3b0*,_|
		//|fp=0x0060|0x40*,_,_,_,_,_,_,_,_,0x229*,_|
		st := MLoad(st);
		//|fp=0x0060|0x60*,_,_,_,_,_,_,_,_,0xbdb*,_,_,_,0x3b0*,_|
		//|fp=0x0060|0x60*,_,_,_,_,_,_,_,_,0x229*,_|
		st := Dup(st,1);
		//|fp=0x0060|0x60*,_,_,_,_,_,_,_,_,_,0xbdb*,_,_,_,0x3b0*,_|
		//|fp=0x0060|0x60*,_,_,_,_,_,_,_,_,_,0x229*,_|
		st := Dup(st,3);
		//|fp=0x0060|_,0x60*,_,_,_,_,_,_,_,_,_,0xbdb*,_,_,_,0x3b0*,_|
		//|fp=0x0060|_,0x60*,_,_,_,_,_,_,_,_,_,0x229*,_|
		st := Dup(st,2);
		//|fp=0x0060|_,_,0x60*,_,_,_,_,_,_,_,_,_,0xbdb*,_,_,_,0x3b0*,_|
		//|fp=0x0060|_,_,0x60*,_,_,_,_,_,_,_,_,_,0x229*,_|
		st := MStore(st);
		//|fp=0x0060|0x60*,_,_,_,_,_,_,_,_,_,0xbdb*,_,_,_,0x3b0*,_|
		//|fp=0x0060|0x60*,_,_,_,_,_,_,_,_,_,0x229*,_|
		st := Push1(st,0x20);
		//|fp=0x0060|0x20*,0x60*,_,_,_,_,_,_,_,_,_,0xbdb*,_,_,_,0x3b0*,_|
		//|fp=0x0060|0x20*,0x60*,_,_,_,_,_,_,_,_,_,0x229*,_|
		assert (st.Peek(0) + st.Peek(1)) <= (MAX_U256 as u256);
		//|fp=0x0060|0x20*,0x60*,_,_,_,_,_,_,_,_,_,0xbdb*,_,_,_,0x3b0*,_|
		//|fp=0x0060|0x20*,0x60*,_,_,_,_,_,_,_,_,_,0x229*,_|
		st := Add(st);
		//|fp=0x0060|0x80*,_,_,_,_,_,_,_,_,_,0xbdb*,_,_,_,0x3b0*,_|
		//|fp=0x0060|0x80*,_,_,_,_,_,_,_,_,_,0x229*,_|
		st := Swap(st,2);
	...
	{
		var st := st';
		//|fp=0x0060|_,_,0x80*,_,_,_,_,_,_,_,0xbdb*,_,_,_,0x3b0*,_|
		//|fp=0x0060|_,_,0x80*,_,_,_,_,_,_,_,0x229*,_|
		st := Pop(st);
		//|fp=0x0060|_,0x80*,_,_,_,_,_,_,_,0xbdb*,_,_,_,0x3b0*,_|
		//|fp=0x0060|_,0x80*,_,_,_,_,_,_,_,0x229*,_|
		st := Pop(st);
		//|fp=0x0060|0x80*,_,_,_,_,_,_,_,0xbdb*,_,_,_,0x3b0*,_|
		//|fp=0x0060|0x80*,_,_,_,_,_,_,_,0x229*,_|
		st := Push1(st,0x40);
		//|fp=0x0060|0x40*,0x80*,_,_,_,_,_,_,_,0xbdb*,_,_,_,0x3b0*,_|
		//|fp=0x0060|0x40*,0x80*,_,_,_,_,_,_,_,0x229*,_|
		st := MLoad(st);
		//|fp=0x0060|0x60*,0x80*,_,_,_,_,_,_,_,0xbdb*,_,_,_,0x3b0*,_|
		//|fp=0x0060|0x60*,0x80*,_,_,_,_,_,_,_,0x229*,_|
		st := Dup(st,1);
		//|fp=0x0060|_,0x60*,0x80*,_,_,_,_,_,_,_,0xbdb*,_,_,_,0x3b0*,_|
		//|fp=0x0060|_,0x60*,0x80*,_,_,_,_,_,_,_,0x229*,_|
		st := Swap(st,2);
		//|fp=0x0060|0x80*,0x60*,_,_,_,_,_,_,_,_,0xbdb*,_,_,_,0x3b0*,_|
		//|fp=0x0060|0x80*,0x60*,_,_,_,_,_,_,_,_,0x229*,_|
		assert st.Peek(1) <= st.Peek(0);
		//|fp=0x0060|_,_,_,_,_,_,_,_,_,_,0xbdb*,_,_,_,0x3b0*,_|
		//|fp=0x0060|_,_,_,_,_,_,_,_,_,_,0x229*,_|
		st := Sub(st);
		//|fp=0x0060|_,_,_,_,_,_,_,_,_,0xbdb*,_,_,_,0x3b0*,_|
		//|fp=0x0060|_,_,_,_,_,_,_,_,_,0x229*,_|
		st := Swap(st,1);
	...
	requires st'.Operands() == 15 ==> ((st'.Peek(9) == 0xbdb && st'.Peek(13) == 0x3b0))
	{
		...
		//|fp=0x0060|_,_,_,_,0x229*,_|
		st := Swap(st,4);
		//|fp=0x0060|0xbdb*,_,_,_,_,_,_,_,0x3b0*,_|
		//|fp=0x0060|0x229*,_,_,_,_,_|
		st := Swap(st,3);
		//|fp=0x0060|_,_,_,0xbdb*,_,_,_,_,0x3b0*,_|
		//|fp=0x0060|_,_,_,0x229*,_,_|
		st := Pop(st);
		//|fp=0x0060|_,_,0xbdb*,_,_,_,_,0x3b0*,_|
		//|fp=0x0060|_,_,0x229*,_,_|
		st := Pop(st);
	...
	{
		var st := st';
		//|fp=0x0060|_,0xbdb*,_,_,_,_,0x3b0*,_|
		//|fp=0x0060|_,0x229*,_,_|
		st := Pop(st);
		//|fp=0x0060|0xbdb*,_,_,_,_,0x3b0*,_|
		//|fp=0x0060|0x229*,_,_|
	...
	{
		var st := st';
		//|fp=0x0060|_,_,_,_,0x3b0*,_|
		st := JumpDest(st);
		//|fp=0x0060|_,_,_,_,0x3b0*,_|
		st := Swap(st,1);
		//|fp=0x0060|_,_,_,_,0x3b0*,_|
		st := Pop(st);
		//|fp=0x0060|_,_,_,0x3b0*,_|
		st := Swap(st,3);
		//|fp=0x0060|0x3b0*,_,_,_,_|
		st := Swap(st,2);
		//|fp=0x0060|_,_,0x3b0*,_,_|
		st := Pop(st);
		//|fp=0x0060|_,0x3b0*,_,_|
		st := Pop(st);
		//|fp=0x0060|0x3b0*,_,_|
