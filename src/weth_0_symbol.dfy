...
module symbol {
	...
	requires (st'.Peek(1) == 0x80)
	{
		...
		//||_,_*,0x80*,_*,_,_*,_,_*,_,_,_,_,_|
		st := Push2(st,0x0335);
		//||0x335*,_,0x00*,0x80*,_*,_,_*,_,_*,_,_,_,_,_|
		//||0x335*,_,_*,0x80*,_*,_,_*,_,_*,_,_,_,_,_|
	...
	requires (st'.Peek(0) == 0x20 && st'.Peek(2) == 0x80)
	{
		...
		//||_*,0x80*,_*,_,_*,_,_*,_,_,_,_,_|
		st := Push2(st,0x031a);
		//||0x31a*,_*,0x80*,_*,_,_*,_,_*,_,_,_,_,_|
	...
	requires (st'.Peek(2) == 0x1 && st'.Peek(3) == 0x2f5)
	{
		...
		assert (st.Peek(0) + st.Peek(1)) <= (MAX_U256 as u256);
		//|fp=0x0060|_,_,0x60*,_,0x01*,0x2f5*,_|
		st := Add(st);
		//|fp=0x0060|_,0x60*,_,0x01*,0x2f5*,_|
		st := Push1(st,0x40);
	...
	requires (st'.Peek(0) == 0x60 && st'.Peek(2) == 0x1 && st'.Peek(3) == 0x60 && st'.Peek(4) == 0x2f5)
	{
		...
		st := Dup(st,3);
		//||0x01*,0x80*,_,_,0x60*,0x2f5*,_|
		st := Dup(st,1);
		//||0x01*,0x01*,0x80*,_,_,0x60*,0x2f5*,_|
		st := SLoad(st);
		//||_*,0x01*,0x80*,_,_,0x60*,0x2f5*,_|
		st := Push1(st,0x01);
		//||0x01*,_*,0x01*,0x80*,_,_,0x60*,0x2f5*,_|
		st := Dup(st,2);
		//||_*,0x01*,_*,0x01*,0x80*,_,_,0x60*,0x2f5*,_|
		st := Push1(st,0x01);
	...
	{
		var st := st';
		//||0x01*,_*,0x01*,_*,0x01*,0x80*,_,_,0x60*,0x2f5*,_|
		st := And(st);
		//||_*,0x01*,_*,0x01*,0x80*,_,_,0x60*,0x2f5*,_|
		st := IsZero(st);
		//||_*,0x01*,_*,0x01*,0x80*,_,_,0x60*,0x2f5*,_|
		st := Push2(st,0x0100);
		//||0x100*,_*,0x01*,_*,0x01*,0x80*,_,_,0x60*,0x2f5*,_|
		assert (st.Peek(0) * st.Peek(1)) <= (MAX_U256 as u256);
		//||0x100*,_*,0x01*,_*,0x01*,0x80*,_,_,0x60*,0x2f5*,_|
		st := Mul(st);
		//||_*,0x01*,_*,0x01*,0x80*,_,_,0x60*,0x2f5*,_|
		...
		//||_*,0x01*,_*,0x01*,0x80*,_,_,0x60*,0x2f5*,_|
		st := Sub(st);
		//||_*,_*,0x01*,0x80*,_,_,0x60*,0x2f5*,_|
		st := And(st);
		//||_*,0x01*,0x80*,_,_,0x60*,0x2f5*,_|
		st := Push1(st,0x02);
		//||0x02*,_*,0x01*,0x80*,_,_,0x60*,0x2f5*,_|
		st := Swap(st,1);
	...
	{
		var st := st';
		//||_*,0x02*,0x01*,0x80*,_,_,0x60*,0x2f5*,_|
		st := Div(st);
		//||_*,0x01*,0x80*,_,_,0x60*,0x2f5*,_|
		st := Dup(st,1);
		//||_,_*,0x01*,0x80*,_,_,0x60*,0x2f5*,_|
		...
		//||_,_*,0x01*,0x80*,_,_,0x60*,0x2f5*,_|
		st := Push2(st,0x0bc6);
		//||0xbc6*,_,_*,0x01*,0x80*,_,_,0x60*,0x2f5*,_|
		...
		//||_,_*,0x01*,0x80*,_,_,0x60*,0x2f5*,_|
		st := Push1(st,0x1f);
		//||_,_,_*,0x01*,0x80*,_,_,0x60*,0x2f5*,_|
		st := Lt(st);
	...
	requires (st'.Peek(2) == 0x1 && st'.Peek(3) == 0x80 && st'.Peek(6) == 0x60 && st'.Peek(7) == 0x2f5)
	{
		...
		//||_,_*,0x01*,0x80*,_,_,0x60*,0x2f5*,_|
		st := Push2(st,0x0b9b);
		//||0xb9b*,_,_*,0x01*,0x80*,_,_,0x60*,0x2f5*,_|
		assume st.IsJumpDest(0xb9b);
		st := JumpI(st);
		if st.PC() == 0xb9b { st := block_0_0x0b9b(st); return st;}
		//||_,0x01*,0x80*,_,_,0x60*,0x2f5*,_|
		st := Push2(st,0x0100);
		//||0x100*,_,0x01*,0x80*,_,_,0x60*,0x2f5*,_|
		st := Dup(st,1);
		//||0x100*,0x100*,_,0x01*,0x80*,_,_,0x60*,0x2f5*,_|
		st := Dup(st,4);
		//||0x01*,0x100*,0x100*,_,_,0x80*,_,_,0x60*,0x2f5*,_|
		st := SLoad(st);
		//||_*,0x100*,0x100*,_,_,0x80*,_,_,0x60*,0x2f5*,_|
		st := Div(st);
		//||_*,0x100*,_,_,0x80*,_,_,0x60*,0x2f5*,_|
		assert (st.Peek(0) * st.Peek(1)) <= (MAX_U256 as u256);
		//||_,_,_,_,0x80*,_,_,0x60*,0x2f5*,_|
		st := Mul(st);
	...
	{
		var st := st';
		//||_,_,_,0x80*,_,_,0x60*,0x2f5*,_|
		st := Dup(st,4);
		//||_,_,_,_,0x80*,_,_,0x60*,0x2f5*,_|
		st := MStore(st);
		//||_,_,0x80*,_,_,0x60*,0x2f5*,_|
		st := Swap(st,2);
		//||0x80*,_,_,_,_,0x60*,0x2f5*,_|
		st := Push1(st,0x20);
		//||0x20*,0x80*,_,_,_,_,0x60*,0x2f5*,_|
		assert (st.Peek(0) + st.Peek(1)) <= (MAX_U256 as u256);
	...
	requires (st'.Peek(2) == 0x80 && st'.Peek(5) == 0x60 && st'.Peek(6) == 0x2f5)
	{
		...
		//||_*,_,0x80*,_,_,0x60*,0x2f5*,_|
		st := Dup(st,3);
		//||0x80*,_*,_,0x80*,_,_,0x60*,0x2f5*,_|
		assert (st.Peek(0) + st.Peek(1)) <= (MAX_U256 as u256);
		//||0x80*,_*,_,0x80*,_,_,0x60*,0x2f5*,_|
		st := Add(st);
		//||_*,_,0x80*,_,_,0x60*,0x2f5*,_|
	...
	{
		var st := st';
		//||0x20*,0x80*,_*,_,_,0x60*,0x2f5*,_|
		st := Push1(st,0x00);
		//||0x00*,0x20*,0x80*,_*,_,_,0x60*,0x2f5*,_|
		st := Keccak256(st);
		//||_*,0x80*,_*,_,_,0x60*,0x2f5*,_|
		st := Swap(st,1);
	...
	requires (st'.Peek(5) == 0x60 && st'.Peek(6) == 0x2f5)
	{
		...
		//||_*,_*,_*,_,_,0x60*,0x2f5*,_|
		st := Dup(st,2);
		//||_,0x80*,_*,_*,_,_,0x60*,0x2f5*,_|
		//||_,_*,_*,_*,_,_,0x60*,0x2f5*,_|
		st := SLoad(st);
		//||_,0x80*,_*,_*,_,_,0x60*,0x2f5*,_|
		//||_,_*,_*,_*,_,_,0x60*,0x2f5*,_|
		st := Dup(st,2);
		//||_,_,0x80*,_*,_*,_,_,0x60*,0x2f5*,_|
		...
		//||_*,_*,_*,_,_,0x60*,0x2f5*,_|
		st := Push1(st,0x01);
		//||0x01*,_*,0x80*,_*,_,_,0x60*,0x2f5*,_|
		//||0x01*,_*,_*,_*,_,_,0x60*,0x2f5*,_|
		assert (st.Peek(0) + st.Peek(1)) <= (MAX_U256 as u256);
		//||0x01*,_*,0x80*,_*,_,_,0x60*,0x2f5*,_|
		//||0x01*,_*,_*,_*,_,_,0x60*,0x2f5*,_|
		st := Add(st);
	...
	requires (st'.Peek(6) == 0x60 && st'.Peek(7) == 0x2f5)
	{
		...
		//||_,_*,_*,_*,_,_,0x60*,0x2f5*,_|
		st := Push2(st,0x0ba9);
		//||0xba9*,_,_*,_*,_*,_,_,0x60*,0x2f5*,_|
