...
module balanceOf {
	...
	// Static stack items
	requires (st'.Peek(1) == 0x4 && st'.Peek(2) == 0x4 && st'.Peek(3) == 0x2cc)
	{
		var st := st';
		// |fp=0x0060|_,0x04,0x04,0x2cc,_|
		st := Swap(st,1);
		// |fp=0x0060|0x04,_,0x04,0x2cc,_|
		st := Push1(st,0x20);
		// |fp=0x0060|0x20,0x04,_,0x04,0x2cc,_|
		st := Add(st);
		// |fp=0x0060|0x24,_,0x04,0x2cc,_|
		st := Swap(st,1);
		// |fp=0x0060|_,0x24,0x04,0x2cc,_|
		st := Swap(st,2);
		// |fp=0x0060|0x04,0x24,_,0x2cc,_|
		st := Swap(st,1);
		// |fp=0x0060|0x24,0x04,_,0x2cc,_|
		st := Pop(st);
	...
	requires (st'.Peek(1) == 0x2cc)
	{
		...
		// |fp=0x0060|_,0x2cc,_|
		st := Push1(st,0x40);
		// |fp=0x0060|0x40,_,0x2cc,_|
		st := MLoad(st);
		// |fp=0x0060|0x60,_,0x2cc,_|
		st := Dup(st,1);
		// |fp=0x0060|0x60,0x60,_,0x2cc,_|
		st := Dup(st,3);
		// |fp=0x0060|_,0x60,0x60,_,0x2cc,_|
		st := Dup(st,2);
		// |fp=0x0060|0x60,_,0x60,0x60,_,0x2cc,_|
		st := MStore(st);
		// |fp=0x0060|0x60,0x60,_,0x2cc,_|
		st := Push1(st,0x20);
	...
	// Static stack items
	requires (st'.Peek(0) == 0x20 && st'.Peek(1) == 0x60 && st'.Peek(2) == 0x60 && st'.Peek(4) == 0x2cc)
	{
		var st := st';
		// |fp=0x0060|0x20,0x60,0x60,_,0x2cc,_|
		st := Add(st);
		// |fp=0x0060|0x80,0x60,_,0x2cc,_|
		st := Swap(st,2);
		// |fp=0x0060|_,0x60,0x80,0x2cc,_|
		st := Pop(st);
		...
		// |fp=0x0060|0x60,0x80,0x2cc,_|
		st := Dup(st,1);
		// |fp=0x0060|0x60,0x60,0x80,0x2cc,_|
		st := Swap(st,2);
	...
	requires st'.Operands() == 5
	// Static stack items
	requires (st'.Peek(0) == 0x80 && st'.Peek(1) == 0x60 && st'.Peek(2) == 0x60 && st'.Peek(3) == 0x2cc)
	{
		var st := st';
		// |fp=0x0060|0x80,0x60,0x60,0x2cc,_|
		st := Sub(st);
		// |fp=0x0060|0x20,0x60,0x2cc,_|
		st := Swap(st,1);
		// |fp=0x0060|0x60,0x20,0x2cc,_|
		st := Return(st);
		return st;
	}
	...
	requires (st'.Peek(1) == 0x2cc)
	{
		...
		// |fp=0x0060|_,0x2cc,_|
		st := Push1(st,0x03);
		// |fp=0x0060|0x03,_,0x2cc,_|
		st := Push1(st,0x20);
		// |fp=0x0060|0x20,0x03,_,0x2cc,_|
		st := MStore(st);
		// |fp=0x0060|_,0x2cc,_|
		st := Dup(st,1);
		// |fp=0x0060|_,_,0x2cc,_|
		st := Push1(st,0x00);
		// |fp=0x0060|0x00,_,_,0x2cc,_|
		st := MStore(st);
		// |fp=0x0060|_,0x2cc,_|
		st := Push1(st,0x40);
	...
	// Static stack items
	requires (st'.Peek(0) == 0x40 && st'.Peek(2) == 0x2cc)
	{
		var st := st';
		// |fp=0x0060|0x40,_,0x2cc,_|
		st := Push1(st,0x00);
		// |fp=0x0060|0x00,0x40,_,0x2cc,_|
		st := Keccak256(st);
		// |fp=0x0060|_,_,0x2cc,_|
		st := Push1(st,0x00);
		// |fp=0x0060|0x00,_,_,0x2cc,_|
		st := Swap(st,2);
		// |fp=0x0060|_,_,0x00,0x2cc,_|
		st := Pop(st);
		// |fp=0x0060|_,0x00,0x2cc,_|
		st := Swap(st,1);
		// |fp=0x0060|0x00,_,0x2cc,_|
		st := Pop(st);
		// |fp=0x0060|_,0x2cc,_|
		st := SLoad(st);
